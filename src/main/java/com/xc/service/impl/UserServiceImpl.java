  ...
  implements IUserService
{
  ...
  public ServerResponse auth(String realName, String idCard, String img1key, String img2key, String img3key, HttpServletRequest request)
  {
    ...
    if (((0 != user.getIsActive().intValue() ? 1 : 0) & (3 != user.getIsActive().intValue() ? 1 : 0)) != 0) {
  ...
  public void ForceSellOneStockTask()
  {
    ...
    for (int i = 0; i < userIdList.size(); i++)
    {
      ...
      if (user != null)
      {
        ...
        for (UserPosition position : userPositions)
        {
          PositionProfitVO positionProfitVO = this.iUserPositionService.getPositionProfitVO(position);
          

          BigDecimal user_force_amt = position.getOrderTotalPrice().divide(new BigDecimal(position.getOrderLever().intValue())).add(position.getMarginAdd()).multiply(force_stop_percent);
          BigDecimal fu_user_force_amt = user_force_amt.negate();
          log.info("用户强制平仓线金额 = {}", user_force_amt);
  ...
  public void ForceSellMessageTask()
  {
    ...
    for (int i = 0; i < userIdList.size(); i++)
    {
      ...
      if (user != null)
      {
        ...
        {
          log.info("强制平仓该用户所有的持仓单");
          int count = this.iSiteMessageService.getIsDayCount(userId, "股票预警");
          if (count == 0)
          {
    ...
    for (int i = 0; i < userIdList.size(); i++)
    {
      ...
      if (user != null)
      {
        ...
        if (isProfit)
        {
          ...
          int count = this.iSiteMessageService.getIsDayCount(userId, "指数预警");
          if (count == 0)
          {
            SiteMessage siteMessage = new SiteMessage();
            siteMessage.setUserId(userId);
  ...
  public void ForceSellFuturesMessageTask()
  {
    ...
    for (int i = 0; i < userIdList.size(); i++)
    {
      ...
      if (user != null)
      {
        ...
        {
          log.info("强制平仓用户 {} 所有的 期货 持仓单", user.getId());
          int count = this.iSiteMessageService.getIsDayCount(userId, "期货预警");
          if (count == 0)
          {
            SiteMessage siteMessage = new SiteMessage();
            siteMessage.setUserId(userId);
  ...
  {
    String idWithPwd = this.userMapper.findIdWithPwd(phone);
    if (idWithPwd == null) {
  ...
  public ServerResponse updateWithPwd(String with_pwd, String phone)
  {
    ...
    String withPwd = this.userMapper.findWithPwd(with_pwd);
    if (withPwd != null) {
      return ServerResponse.createByErrorMsg("您已经添加了提现密码！");
    }
    int i = this.userMapper.updateWithPwd(with_pwd, phone);
    if (i > 0) {
  ...
  {
    this.userMapper.updateUserAmt(amt, user_id);
  }
}
 ...
 * Qualified Name:     com.xc.service.impl.UserServiceImpl
