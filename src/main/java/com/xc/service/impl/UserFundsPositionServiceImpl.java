...
public class UserFundsPositionServiceImpl
  implements IUserFundsPositionService
{
  ...
  public ServerResponse insert(UserFundsPosition model, HttpServletRequest request)
  {
    ...
    ret = this.userFundsPositionMapper.insert(model);
    if (ret > 0) {
  ...
  {
    int ret = this.userFundsPositionMapper.update(model);
    return ret > 0 ? ret : 0;
  }
  ...
  public ServerResponse save(UserFundsPosition model)
  {
    ...
    if ((model != null) && (model.getId().intValue() > 0)) {
      ret = this.userFundsPositionMapper.update(model);
    } else {
      ret = this.userFundsPositionMapper.insert(model);
    }
    if (ret > 0) {
  ...
  @Transactional
  public ServerResponse sellFunds(String positionSn, int doType)
    throws Exception
  {
 ...
 * Qualified Name:     com.xc.service.impl.UserFundsPositionServiceImpl
