  ...
  implements IPayService
{
  ...
  public ServerResponse juhenewpay(String paramString1, String paramString2, HttpServletRequest request)
    throws Exception
  {
    User user = this.iUserService.getCurrentRefreshUser(request);
  ...
  public ServerResponse juheh5pay(String paramString1, String paramString2, HttpServletRequest request)
    throws Exception
  {
    User user = this.iUserService.getCurrentRefreshUser(request);
    String orderid = user.getId() + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
  ...
  public ServerResponse juhenewpayNotify(HttpServletRequest request)
    throws UnsupportedEncodingException
  {
    ...
    map.put("currency_type", request.getParameter("currency_type"));
    map.put("goods_name", request.getParameter("goods_name"));
    map.put("out_trade_no", out_trade_no);
    map.put("pay_id", request.getParameter("pay_id"));
    map.put("pay_no", request.getParameter("pay_no"));
    map.put("payment", request.getParameter("payment"));
    map.put("resp_code", request.getParameter("resp_code"));
    map.put("resp_desc", URLDecoder.decode(request.getParameter("resp_desc"), "UTF-8"));
    map.put("sign_type", request.getParameter("sign_type"));
    map.put("tran_amount", request.getParameter("tran_amount"));
    map.put("version", request.getParameter("version"));
    String _sign = CmcPayOuterRequestUtil.getSign(map, "1ee6fa81e2471cacdc14714f704bf5ec11");
    if (("00".equals(request.getParameter("resp_code"))) && (_sign.equals(request.getParameter("sign")))) {
      this.iUserService.updateUserAmt((Double)map.get("amount"), Integer.parseInt(arr[0]));
    }
    return null;
  }
}
 ...
 * Qualified Name:     com.xc.service.impl.PayServiceImpl
