...
public class StockFuturesServiceImpl
  implements IStockFuturesService
{
  ...
  public ServerResponse queryList(HttpServletRequest request)
  {
    ...
    for (StockFutures stockFutures : list)
    {
      ...
      if (user == null) {
        ...
        stockFuturesListVO.setIsOption(this.iStockOptionService.isMyOption(user.getId(), stockFutures.getFuturesCode()));
      }
      stockFuturesListVOS.add(stockFuturesListVO);
    }
    return ServerResponse.createBySuccess(stockFuturesListVOS);
 ...
 * Qualified Name:     com.xc.service.impl.StockFuturesServiceImpl
