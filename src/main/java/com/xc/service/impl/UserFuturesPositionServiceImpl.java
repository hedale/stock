...
public class UserFuturesPositionServiceImpl
  implements IUserFuturesPositionService
{
  ...
  @Transactional
  public ServerResponse buyFutures(Integer futuresId, Integer buyNum, Integer buyType, Integer lever, HttpServletRequest request)
    throws Exception
  {
    ...
    log.info("futures 是否在上午交易时间 = {} 是否在下午交易时间 = {}是否在下午交易时间2 = {}", new Object[] { Boolean.valueOf(am_flag), Boolean.valueOf(pm_flag), Boolean.valueOf(pm_flag2) });
    ...
    BigDecimal all_deposit_amt = new BigDecimal(stockFutures.getDepositAmt().intValue()).multiply(new BigDecimal(buyNum.intValue())).divide(new BigDecimal(lever.intValue())).setScale(4, 2);
  ...
  public ServerResponse sellFutures(String positionSn, int doType)
    throws Exception
  {
  ...
  private UserFuturesPositionVO assembleUserFuturesPositionVO(UserFuturesPosition userFuturesPosition)
  {
    ...
    BigDecimal eachPoint = new BigDecimal("1");
    StockFutures stockFutures = this.stockFuturesMapper.selectFuturesByCode(userFuturesPosition.getFuturesCode());
    if (stockFutures != null) {
      eachPoint = stockFutures.getEachPoint();
    }
 ...
 * Qualified Name:     com.xc.service.impl.UserFuturesPositionServiceImpl
