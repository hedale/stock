...
public class QqStockApi
{
  ...
  public static ServerResponse<MinDataVO> getGpStockDayK(Stock stock)
  {
    ...
    int size = Integer.valueOf(PropertiesUtil.getProperty("qq.k.min.max.size")).intValue();
    if (list.size() > size) {
  ...
  public static ServerResponse<MinDataVO> getQqStockDayK(StockFutures stock)
  {
    ...
    int size = Integer.valueOf(PropertiesUtil.getProperty("sina.futures.day.min.max.size")).intValue();
    if (list.size() > size) {
      list = list.subList(list.size() - size - 1, list.size());
    }
    MinDataVO minDataVO = new MinDataVO();
 ...
 * Qualified Name:     com.xc.utils.stock.qq.QqStockApi
